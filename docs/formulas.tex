...
Consideriamo un wormhole con \\( b_0 = 14.1347 \\) (corrispondente al primo zero non banale di \\( \\zeta(s) \\)). La metrica diventa:
\\begin{equation}
ds^2 = -e^{-\\frac{28.2694}{r}} dt^2 + \\frac{dr^2}{1 - \\frac{14.1347}{r}} + r^2 (d\\theta^2 + \\sin^2\\theta \\, d\\phi^2).
\\end{equation}
Questa metrica può essere utilizzata per simulare la geometria del wormhole nel software RIQA.
...
\\subsection{Esempio Pratico}
Calcoliamo i primi 10 zeri non banali di \\( \\zeta(s) \\) e utilizziamoli per determinare i raggi della gola \\( b_0 \\) di altrettanti wormhole:
\\begin{equation}
b_0^{(n)} = t_n, \\quad n = 1, 2, \\ldots, 10.
\\end{equation}
Questi valori possono essere utilizzati per simulare diverse configurazioni di wormhole nel software RIQA.
