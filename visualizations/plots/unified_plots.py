...
def create_analysis_plots(df, stats):
    ...
    for file_key in df["File Key"].unique():
        ...
        # Esporta in PNG, PDF e SVG
        pio.write_image(fig, f"{file_key}_analysis.png", format="png")
        pio.write_image(fig, f"{file_key}_analysis.pdf", format="pdf")
        pio.write_image(fig, f"{file_key}_analysis.svg", format="svg")
    ...
    # Esporta in PNG, PDF e SVG
    pio.write_image(fig_dist, "validation_summary_6205.png", format="png")
    pio.write_image(fig_dist, "validation_summary_6205.pdf", format="pdf")
    pio.write_image(fig_dist, "validation_summary_6205.svg", format="svg")

     #Aggiungi più opzioni di personalizzazione, come la scelta di colori, stili e temi
    def create_analysis_plots(df, stats, theme: str = 'plotly_white'):
    # Grafico per ogni file
    for file_key in df["File Key"].unique():
        file_df = df[df["File Key"] == file_key]
        fig = go.Figure()
